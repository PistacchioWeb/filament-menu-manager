@php use PistacchioWeb\FilamentMenuManager\Models\MenuItem; @endphp
@props(['item'])
...
<li wire:key="{{ $item->getKey() }}" data-sortable-item="{{ $item->getKey() }}" x-data="{ open: $persist(true).as('menu-item-' + {{ $item->getKey() }}) }">
    <div
        ...
        <div class="flex flex-1 items-center gap-2 truncate">
            ...
            @if ($hasChildren)
                <x-filament::icon-button icon="heroicon-o-chevron-right" x-on:click="open = !open"
                    x-bind:title="open ? '{{ trans('filament-menu-manager::menu-manager.items.collapse') }}' :
                        '{{ trans('filament-menu-manager::menu-manager.items.expand') }}'"
                    color="gray" class="transition duration-200 ease-in-out" x-bind:class="{ 'rotate-90': open }"
                    size="sm" />
            @endif

            @if (\PistacchioWeb\FilamentMenuManager\FilamentMenuManagerPlugin::get()->isIndentActionsEnabled())
